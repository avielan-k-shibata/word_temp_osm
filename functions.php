...
if(!function_exists('portfolio_styles')) {
	...
	function portfolio_styles() {
		...
		if ( is_page() ){
			wp_enqueue_style( 'contents2', get_stylesheet_directory_uri() . '/css/osmosis_contactForm.css', array(),  true);
			wp_enqueue_style( 'googlefont_01', 'https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700&display=swap', array(),  true);
		}
	}
}
...
add_action( 'init', 'create_styling_type' );

function create_styling_type() {
